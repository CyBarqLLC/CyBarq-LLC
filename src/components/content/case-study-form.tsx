...
import { PRACTICES, type ImpactItem } from "@/lib/validation/content";
...
type Props = {
  ...
  defaults: Partial<Tables<"case_studies">>;
...
const NARRATIVE = ["challenge", "solution", "implementation", "outcome"] as const;

export function CaseStudyForm({ action, defaults, impactItems, internalProjects, publicBase, mode }: Props) {
  ...
  return (
    <ServerActionForm action={formAction} result={result} className="flex flex-col gap-10" noValidate>
        ...
        <LangGrid
          en={
            <>
              ...
              <Field label={t("industry")} htmlFor="industry_en" error={fieldError(result, "industry_en")}>
                <Input id="industry_en" name="industry_en" defaultValue={defaults.industry_en ?? ""} maxLength={120} />
              </Field>
            </>
          }
          ar={
            <>
              ...
              <Field label={t("industry")} htmlFor="industry_ar" error={fieldError(result, "industry_ar")}>
                <Input id="industry_ar" name="industry_ar" defaultValue={defaults.industry_ar ?? ""} maxLength={120} />
              </Field>
            </>
          }
        />
      ...
      {NARRATIVE.map((key) => (
        <Section key={key} title={t(`narrative.${key}`)}>
          <LangGrid
            en={
              <Field label={t(`narrative.${key}`)} htmlFor={`${key}_en`} error={fieldError(result, `${key}_en`)}>
                <RichTextEditor name={`${key}_en`} defaultValue={defaults[`${key}_en` as const]} dir="ltr" placeholder={te("placeholder")} labels={editorLabels} />
              </Field>
            }
            ar={
              <Field label={t(`narrative.${key}`)} htmlFor={`${key}_ar`} error={fieldError(result, `${key}_ar`)}>
                <RichTextEditor name={`${key}_ar`} defaultValue={defaults[`${key}_ar` as const]} dir="rtl" placeholder={te("placeholder")} labels={editorLabels} />
              </Field>
            }
          />
        </Section>
      ...
      <Section title={t("impact.title")} description={t("impact.description")}>
        <ImpactEditor name="impact" defaultItems={impactItems} error={fieldError(result, "impact")} />
      </Section>
      ...
      <Section title={t("sections.classification")}>
        <div className="grid gap-5 md:grid-cols-3">
          <Field label={t("practice")} htmlFor="practice" error={fieldError(result, "practice")} required>
            <NativeSelect id="practice" name="practice" defaultValue={defaults.practice ?? "mixed"}>
              {PRACTICES.map((p) => (
      ...
      <CoverSection result={result} target="case-studies" defaults={defaults} publicBase={publicBase} />
