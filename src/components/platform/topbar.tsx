    ...
    <header className="sticky top-0 z-30 flex h-(--nav-height) items-center gap-2 border-b border-fog bg-white px-3 safe-pt sm:px-5">
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent side="start" title={t("menu")} className="p-0 [&>div:last-child]:p-0">
          <Sidebar sections={sections} commands={commands} onNavigate={() => setOpen(false)} namespace={variant === "portal" ? "portal.nav" : "platform.nav"} homeHref={base} />
        </SheetContent>
      </Sheet>
      <div className="ms-auto flex items-center gap-1 sm:gap-2">
        <CommandSearch items={commands} className="lg:hidden" labels={{ placeholder: t("searchPlaceholder"), empty: t("searchEmpty"), title: t("search"), open: t("search") }} />
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-56">
            ...
            <DropdownMenuItem asChild>
              <Link href={`${base}/settings`}><Icon name="account" className="size-4" /> {t("profile")}</Link>
            </DropdownMenuItem>
            ...
            <DropdownMenuItem onSelect={() => void signOut()} destructive>
              <Icon name="signOut" className="size-4" /> {t("signOut")}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
