...
export default async function ContactPage({ params, searchParams }: Props) {
  ...
  return (
    <>
      ...
      <div className="container-page grid gap-12 border-t border-fog py-14 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] lg:gap-20 sm:py-20">
        <section aria-labelledby="channels-heading" className="flex flex-col gap-8">
          <h2 id="channels-heading" className="text-h2">{t("channelsEyebrow")}</h2>
          <ul className="flex flex-col divide-y divide-fog border-y border-fog">
            {channels.map((c) => (
              <li key={c.key} className="flex flex-col gap-1 py-5">
                ...
                <a href={`mailto:${c.email}`} className="text-azure hover:underline underline-offset-4">{c.email}</a>
                <p className="text-small text-slate">{t(`channels.${c.key}.description`)}</p>
              </li>
            ))}
          </ul>
          <p className="text-small text-slate">{t("location")}</p>
        </section>

        <section aria-labelledby="form-heading">
          <p className="text-small text-slate">{t("form.eyebrow")}</p>
          <h2 id="form-heading" className="text-h2 mt-1 mb-8">{t("form.title")}</h2>
