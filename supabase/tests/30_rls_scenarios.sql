...
select tests.expect_ok('insert into public.user_roles (user_id, role_key, granted_by) values (''00000000-0000-0000-0000-00000000a009'', ''developer'', ''00000000-0000-0000-0000-00000000a002'')', 'admin: can assign a role within their own permissions');
select tests.expect_rows('select 1 from public.audit_logs where action = ''role.granted'' and entity_id = ''00000000-0000-0000-0000-00000000a009'' and metadata ->> ''role'' = ''developer'' and actor_id = ''00000000-0000-0000-0000-00000000a002''', 1, 'admin: role grant audited with actor');
select tests.expect_error('insert into public.user_roles (user_id, role_key, granted_by) values (''00000000-0000-0000-0000-00000000a009'', ''finance'', ''00000000-0000-0000-0000-00000000a002'')', 'admin: cannot grant a role carrying permissions they lack (finance.write)');
select tests.expect_error('insert into public.user_roles (user_id, role_key) values (''00000000-0000-0000-0000-00000000a002'', ''finance'')', 'admin: cannot grant roles to themselves');
